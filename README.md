...
- 代码区签名：对 `[l2c_sig_a, l2c_sig_b)` 这段机器码做 FNV 校验，启动时采样为基准，
  之后每次进入生成的函数都复查。Frida 或调试器在运行途中装的 inline hook、`int3`
  断点都会改变它
...
- `L2C_GUARD_REPORT=1` 打印测量结果

**GUI 客户端**（`luac2c_flutter/`，Material You / Material Design 3）

一键构建并比对走完"编译字节码 → 转译为 C → gcc 编译 → 运行 → 与 lua.exe 比对"整条链路，
多文件并发处理；另有「仅生成 C 源码」只跑前两步。批量模式下多选或拖入多个 `.lua` 文件依次
