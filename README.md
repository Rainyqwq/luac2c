...
编译生成的 C 请用 `-O0`（客户端也是这么做的）。`-O1/-O2` 编出来的产物行为同样正确，
只是优化器会把受保护区的两个边界标记函数挪到一起，受保护区随后只剩一小段，代码篡改的
...
- 常量池强化：双密钥混合解码，并按内容全池去重，同一个常量只存一份。按 proto 各存
  一份的写法下，一个 60 函数的模块会把 `"print"` 这类共享串存 60 遍；实测 365 条池条目
  降到 8 条，生成的 C 也小了 5%
- 密钥不挂在锁旁边：每个常量解出的明文，除了静态密钥流，还异或两样东西。一是前缀链，
  也就是这一条之前所有密文字节的滚动哈希，改动任何一条密文，它后面所有常量都会解成
  垃圾；二是运行密钥，完好构建上是 0，检出篡改时才变成由本机栈地址、堆地址、时钟和一次
  真实 Lua 调用返回的指针混出来的值。于是没有干净的值可以写回：把标志清零不会恢复正确性，
  只有进程内才存在的那把钥匙才行。错值直接进入后续运算（跳转目标、循环次数、表键），
  不会汇进一个没人看的标志位
- 发布构建（`--l2c-release`）：生成的 C 里一条注释都不留，标识符统一成 `aN`，42 个
  API 宏的定义顺序打乱，每个宏把同一把密钥的还原写成不同的等价形式（`x^y`、
  `(x|y)-(x&y)`、`(x+y)-2*(x&y)`、`(x&~y)+(y&~x)` 各占一部分）。读通一个调用点，
  对下一个没有帮助
- 控制流扁平化：状态机分发器改写全部控制流
- 函数切分：函数体切成由指针表调用的块函数
- 整数 MBA 混淆（`--no-mba` 关闭）：寄存器索引 `b + idx + 1` 与状态机解码 `st - base`
  改用混合布尔算术恒等式书写，三种形式里随机取一种：`x+y == (x^y)+2*(x&y)`、
  `x+y == (x|y)+(x&y)`、`x+y == x-~y-1`。调用点里 `idx` 是常量，编译器会折叠回同一次
  加法，运行期零开销，但反编译器读不回"基址加下标"
- 常量按需解密 + 用完擦除（`--no-wipe` 退回旧的"启动即全量展开"）：字符串不再在启动时
  被解码成一张 Lua 表，而是每次用到时才从 blob 解到栈上暂存区，入栈后立刻清零
  （`volatile` 写入，防止被优化掉）。任何时刻 dump 内存，拿得到的只有当前正在用的那一个
  常量
...
- 代码区校验：启动时把受保护区切成若干个 1 KiB 窗口各存一份摘要，之后每进入一次生成函数
  ...
  单次开销反而更低，只算一个窗口，不用扫过整段
- 守卫代码自保护：这些检查函数原先排在受保护区之前，改掉 `l2c_guard_poll` 让它恒返回
  "干净"，代码签名毫无反应，整个机制一句话就能废掉。现在它们由 `l2c_grd_a` / `l2c_grd_b`
  框住并单独取摘要，改动检查者本身即被检出。代价是写操作要挪出这个区间：运行期会被写入
  的状态变量都在标记之前，否则程序改自己的变量就会被判成补丁
- 跨区间交叉校验：检查器每次调用都会推进一个令牌，入口处（位于另一个受保护区里）
  比较调用前后的令牌。于是"把检查器换成一个空壳让它直接返回干净"这种攻击会被调用方发现，
  而调用方自己要被动，又会被窗口轮转抓到。两个区间互相盯着
- 硬件断点检测：调试寄存器（DR0–DR3 / DR7）可以给代码下硬件断点而不写一个字节，
  因此任何内存校验都看不见它，这是唯一一类落在校验盲区里的监控手段。当前线程直接读寄存器，
  其他线程在 200ms 节流的那一档里遍历（断点是线程级状态）
- 标志位镜像：`l2c_gflags` 有一份取反的影子副本，每次检查时比对。见到"找出标志变量
  清成 0"这种最常见的绕过手法时，两个变量就对不上了
- 入口字节检查：每个生成函数在入口处看自己第一个字节是不是 `0xCC`（软件断点 /
  钩子桩的首字节）。没有任何编译器会把 `int3` 当函数序言，所以正常构建不会误报，
  而钩子还没等到窗口轮转到这个函数就已经被发现
...
- 代码页可写检测：Frida 下钩子前必须先把代码页改成可写，所以"可写"是个信号。但有些
  链接布局下映像首页本来就是 RWX（一个 4 KiB 页被 .text 和可写段共享），所以判据是
  启动时先记下基线，只把"从只读变成可写"这次翻转当作证据
...
- 反制方式：命中任意一项即置位标志（位：`1` 调试器 `2` 被跟踪 `4` frida 模块 `8` frida 线程
  ...
  `512` 时间差 `1024` 硬件断点 `2048` 入口字节可疑），同时污染常量池密钥与栈帧基址。
  程序照常跑完并正常退出，读写的寄存器却全部错位；这里没有可以 nop 掉的分支，
...
- 开销：守卫是每进入一次生成函数就验一个窗口，早先按字节逐个哈希，200 万次调用的
  ...
  同一个热循环：不混淆 341ms、混淆不守卫约 400ms、混淆加守卫 694ms，
  也就是 18 倍降到 2 倍。跨区间令牌校验又添了约 100ms，换来的是"换掉检查器本身"这条
  路子不再有效。守卫区的采样与覆盖不变，篡改依旧被检出

验证方式（三种攻击都用 `-DL2C_SELFTEST=N` 实测过，正常构建不含这些代码）：
`N=1` 翻转生成代码区里的一个字节、`N=2` 翻转**守卫区**里的一个字节、`N=3` 给当前线程下一枚
硬件断点（不写任何内存）。三者都让程序给出失配响应、输出与 `lua.exe` 不再一致，
而不带 `N` 的干净构建完全一致。
文件侧则用 `--l2c-sig` + `--sign` 固化后改一个字节，同样被检出。
输入侧另有 400 轮模糊测试（355 个畸形输入被拒收，0 崩溃）。

畸形字节码的战线在 `validate_proto()`：任何下标、窗口、跳转目标越界都在这里被拒，
带着"第几条指令、哪个字段、越界多少"的说明退出。生成器不会把畸形输入翻译成一份控制流
被悄悄改错的文件。控制流指令的跳转偏移（FORPREP / FORLOOP / TFORLOOP 的 Bx）
...
一键构建并比对依次做"编译字节码 → 转译为 C → gcc 编译 → 运行 → 与 lua.exe 比对"，
多文件并发；另有「仅生成 C 源码」只跑前两步。批量模式下多选或拖入多个 `.lua` 文件依次
...
| `check_combos.py` | 14 种开关组合 × 2 个优化级别：每种都要能编译并跑出正确结果        |
