...
- 常量池强化：双密钥混合的常量池解码，并且全池去重 —— 同一个常量只存一份。
  按 proto 各存一份的写法，一个 60 函数的模块会把 `"print"` 这类共享串存 60 遍；
  实测 365 条池条目降到 8 条，生成的 C 也小了 5%
...
- 开销：守卫是每进入一次生成函数就验一个窗口，早先按字节逐个哈希，200 万次调用的
  热循环里要跑二十多亿次迭代，整体比不混淆的版本慢 18 倍。现在窗口按机器字折叠
  （八分之一的工作量）、每四次入口验一个窗口、环境与内存页扫描按时间节流到 200ms
  一次（原来是每 256 次调用一次，进程快照本身就吃掉了四分之三的运行时间）。
  同一个热循环：不混淆 341ms、混淆不守卫约 400ms、混淆加守卫 590ms，
  也就是 18 倍降到 1.7 倍。守卫区的采样与覆盖不变，篡改依旧被检出
...
畸形字节码的战线在 `validate_proto()`：任何下标、窗口、跳转目标越界都在这里被拒，
带着"第几条指令、哪个字段、越界多少"的说明退出，而不是让生成器把它翻译成一份
控制流被悄悄改错的文件。控制流指令的跳转偏移（FORPREP / FORLOOP / TFORLOOP 的 Bx）
曾经是漏网的一类，现在一并校验。定向属性测试把这三类指令的操作数逐个改坏，
要求"要么明确拒绝，要么生成的 C 仍能编译"：修复前 37 个位置放过 22 个，
现在拒掉 35 个，剩下 2 个的产物也能正常编译。
