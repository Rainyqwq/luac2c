...
- 硬件断点检测：调试寄存器（DR0–DR3 / DR7）可以给代码下硬件断点而**一个字节都不写**，
  因此任何内存校验都看不见它，这是唯一一类校验天然盲区的监控手段。当前线程直接读寄存器，
...
- 开销：守卫是每进入一次生成函数就验一个窗口，早先按字节逐个哈希，200 万次调用的
  ...
  也就是 18 倍降到 2 倍。加上跨区间令牌校验之后比上一版贵约 100ms，是它换来的：
  换掉检查器本身不再有效。守卫区的采样与覆盖不变，篡改依旧被检出
